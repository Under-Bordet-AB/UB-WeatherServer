...
// work function for the scheduler, it justs accepts new connections
void TCPServer_TaskWork(void* _Context, uint64_t _MonTime) {
    TCPServer* _Server = (TCPServer*)_Context;
    int socket_fd = accept(_Server->listen_fd, NULL, NULL);

    if (socket_fd < 0) {
        if (errno == EAGAIN || errno == EWOULDBLOCK) return 0; // No new client, return
        perror("accept");
        return -1;
    }
    // set the new clients socket to non-blocking
    TCPServer_Nonblocking(socket_fd);
    // call the servers onAccept function with the new socket
    int result = _Server->onAccept(socket_fd, _Server->context);
    if (result != 0) close(socket_fd);
    return 0;
}

// No callback function since we are last / at the bottom
...
int TCPServer_Initiate(TCPServer* _Server, const char* _Port, TCPServer_OnAccept _OnAccept, void* _Context) {
    ...
    // NOTE here we add the task of checking for new connections to the scheduler
